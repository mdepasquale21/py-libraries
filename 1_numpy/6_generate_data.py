...
print('\n')
print('Usually we have dataframes in which each row is a sample, and each column a property or feature')
print('In that case we are interested in the mean of each feature that is each column (mean by rows) -> axis=0')
print('Also because columns might have different units...')
print('That would allow us to analyse the distribution of each feature (e.g. histograms)')
print('In our case we would have 10000 observations with 3 features')

print('\n')
print('COVARIANCE OF A MATRIX')
print('If we would like to analyse the covariance of the features in our data')
print('Covariance matrix should be n_features x n_features')
print('np.cov(M)')
cov_matrix = np.cov(M)
print(cov_matrix)
print('shape',cov_matrix.shape)
print('Numpy covariance matrix by default instead treats each column as an observation!!!')
print('This is not the default in all other important python libraries (e.g. sklearn, tensorflow, ..)')
print('In this case we should transpose our matrix before applying np.cov()')
print('np.cov(M.T)')
correct_cov_matrix = np.cov(M.T)
print(correct_cov_matrix)
print('shape',correct_cov_matrix.shape)
print('Or as an alternative to this it is possible to call np.cov(M,rowvar=False) to disable the default behavior')
print('and treat the rows as observations')
print(np.cov(M,rowvar=False))

print('\n')
print('GENERATE RANDOM INTEGERS')
print('With np.random.randint()')
print('np.random.randint(0,10,size=(3,3))')
print('   |')
print('   V')
print(np.random.randint(0,10,size=(3,3)))
print('With np.random.choice()')
